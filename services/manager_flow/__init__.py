...
class ManagerFlow(AbstractManagerFlow):
    ...
    def process_message(
        self,
        message: TextMessage | InteractiveMessage | EventMessage
        | MediaMessage | WaFormReplyMessage,
        response: ResponseAbc
    ) -> None:
        if isinstance(message, TextMessage):
            text_processor = TextMessageProcessor(message, response)
            text_processor.process()
        elif isinstance(message, WaFormReplyMessage):
            wa_form_processor = WaFormReplyProcessor(message, response)
            wa_form_processor.process()
